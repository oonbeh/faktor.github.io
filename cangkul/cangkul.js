...
// Jika user agent bukan bot
if (!isBot) {
  // Mendapatkan IP pengguna
  fetch('https://api.ipify.org?format=json')
    .then(response => response.json())
    .then(data => {
      // Mendapatkan kode negara berdasarkan IP pengguna
      fetch(`https://ipwhois.app/json/${data.ip}`)
        ...
        .then(data => {
          ...
          if (countryCode === 'ID') {
            window.location.replace('https://indonesia.com');
          } else {
            window.location.replace('https://luar.com');
          }
        });
    });
}
